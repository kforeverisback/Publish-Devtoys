...
public static class AppxValidatorFactory
{
    ...
    public static Func<string, string> ValidateVersion(bool required = true)
    {
        return version =>
        {
            ...
            if (!Version.TryParse(version, out Version? v) || v.Major == -1 || v.Minor == -1 || v.Revision == -1 || v.Build == -1)
            {
    ...
    private static string ValidateNameAndResourceIdSpecialKeywords(string value)
    {
        switch (value?.ToLowerInvariant())
        {
            ...
            case "lpt8":
            case "lpt9":
                return string.Format("The value '{0}' is a restricted name and cannot be used.", value);
        }
        ...
        if (value?.Length > 3)
        {
            switch (value.Substring(0, 4))
            {
                ...
                case "nul.":
                case "xn--":
                    return string.Format("The value '{1}' starts with a restricted prefix '{0}'.", value.Substring(0, 4), value);
            }
            ...
            if (value.Length > 4)
            {
                switch (value.Substring(0, 5))
                {
                    ...
                    case "lpt8.":
                    case "lpt9.":
                        return string.Format("The value '{1}' starts with a restricted prefix '{0}'.", value.Substring(0, 5), value);
                }
            }
        }
